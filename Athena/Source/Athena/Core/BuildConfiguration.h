...
// Configuration
	...
	#define ATN_ENABLE_STATS 1
	#define ATN_ENABLE_TRACY 1
#elif ATN_RELEASE
	#define ATN_ENABLE_DEBUGBREAK 1
	...
	#define ATN_ENABLE_STATS 0
	#define ATN_ENABLE_TRACY 0
#endif

#define ATN_ENABLE_LOGGING 1
...
#define TEXT(quote) L##quote
	...
	#define ATN_INTERNAL_ASSERT_IMPL(cond, msg, ...) { if(!(cond)) { ATN_LOG_ERROR(Debug, msg __VA_OPT__(,) __VA_ARGS__); ATN_DEBUGBREAK(); } }
...
#if ATN_ENABLE_CHECKS || ATN_ENABLE_ENSURES
	#define ATN_UNIMPLEMENTED() ATN_INTERNAL_ASSERT_IMPL(false, "Unimplemented: {0} at {1}:{2}", __FUNCSIG__, std::filesystem::path(__FILE__).filename().string(), __LINE__)
#endif
