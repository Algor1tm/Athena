...
namespace Athena
{
		...
		: m_FilePath(filepath)
	{
		...
		if (data)
		{
			...
			{
				ATN_CORE_ERROR_TAG("GLTexture2D", "Texture format not supported(texture = {0}, channels = {1})", filepath, channels);
		...
		{
			ATN_CORE_ERROR_TAG("GLTexture2D", "Failed to load Texture2D from {}!)", m_FilePath);
		}
	}
	...
	GLTexture2D::GLTexture2D(const void* Data, uint32 Width, uint32 Height, bool sRGB, const TextureSamplerCreateInfo& samplerInfo)
	{
		...
		if (data)
		{
			...
			{
				ATN_CORE_ERROR_TAG("GLTexture2D", "Texture format not supported(channels = {})", channels);
				stbi_image_free(data);
				return;
			}
