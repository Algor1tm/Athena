...
namespace Athena
{
	...
	{
	public:
		static Ref<Texture2D> Import(const FilePath& path, bool sRGB = false);
		static Ref<Texture2D> Import(const FilePath& path, const TextureImportOptions& options = TextureImportOptions());

		static Ref<Texture2D> LoadFromMemory(const void* data, uint32 width, uint32 height, const TextureImportOptions& options = TextureImportOptions());
