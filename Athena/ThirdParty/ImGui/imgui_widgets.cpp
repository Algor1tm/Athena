...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float w = (flags & ImGuiComboFlags_NoPreview) ? arrow_size : GetContentRegionAvail().x - 15.f; //------------- ATHENA EDIT -------------
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    if (display_frame)
    {
        ...
        else if (!is_leaf)                                                                     // ----------- ATHENA EDIT -----------
            RenderArrow(window->DrawList, ImVec2(text_pos.x - text_offset_x + padding.x, text_pos.y + 1.f), text_col, is_open ? ImGuiDir_Down : ImGuiDir_Right, 0.90f);
